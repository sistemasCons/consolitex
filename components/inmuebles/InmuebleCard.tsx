    ...
    <>
      <Box
        sx={{
          ...
          cursor: "pointer",
          boxShadow: {
            xs: "0 0 10px rgba(0,0,0,0.1)",
            md: "0 8px 32px 0 rgba(0,0,0,0.3)",
          },
        }}
      ...
      >
        ...
        >
          <Image
            alt={data.nombre}
            src={`https://images.consolitex.org/newImg.php?url=${encodeURIComponent(
              url_inmueble
            )}`}
            layout="fill"
            loading="lazy"
        ...
        <Box sx={{ pt: 2, pr: 2, pl: 2, pb: 0 }}>
          <Box sx={{ display: "flex", justifyContent: "space-between" }}>
            ...
            >
              REF {numberWithDots(Number(data.ref))}, <sub>00</sub>
            </Typography>
            <Typography
          ...
          <Typography
            ...
            sx={{
              "&:before": {
                content: `'${
                  data.nombre.length > 25
                    ? ucfirst(data.nombre.toLocaleLowerCase()).substring(
                        0,
                        25
                      ) + "..."
                    : ucfirst(data.nombre.toLocaleLowerCase())
                }'`,
              },
              "&:hover:before": {
        ...
        >
          ...
          <Box sx={{ display: "flex", alignItems: "center", mr: 1 }}>
            ...
            <Typography variant="subtitle2" sx={{ ml: 0.5 }}>
              {data.banos}{" "}
              {Number(data.banos) > 1 || Number(data.banos) < 1
                ? " baños"
                : " baño"}
            </Typography>
          </Box>
