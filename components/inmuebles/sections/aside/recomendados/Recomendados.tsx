            ...
            <Box sx={{ ...styles.gradientBoxSizes, ...styles.gradiantBoxShadow }}>
                ...
                <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                    <Box sx={{ textAlign: "center", width: 250, mt: 2, position: "relative" }}>
                        <IconButton size="small" sx={styles_local.left} onClick={() => sliderRecomendadoRef.current !== null ? sliderRecomendadoRef?.current.slickPrev() : false}><ChevronLeftRounded /></IconButton>
                        <IconButton size="small" sx={styles_local.right} onClick={() => sliderRecomendadoRef.current !== null ? sliderRecomendadoRef?.current.slickNext() : false}><ChevronRightRounded /></IconButton>
...

                            {
                                related.map((rel: any) => (
                                    <Box component="div" key={rel.data.key}>
                                        <Box
                                            component="div"
                                            sx={styles_local.imageContainer}
                                        >
                                            <CustomImage
                                                upperBoxStyles={styles_local.upperBox}
                                                src={`https://images.consolitex.org/newImg.php?url=${encodeURI(rel.url_inmueble)}`}
...
const styles_local = {
    ...
    left: { position: "absolute", top: "50%", transform: "translateY(-50%)", left: -25, zIndex: 90 },
    right: { position: "absolute", top: "50%", transform: "translateY(-50%)", right: -25, zIndex: 90 },
}