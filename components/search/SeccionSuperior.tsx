...
export const SeccionSuperior: FC<PropsMenuSuperior> = ({
  initialFilter,
  squared,
  toggleSquare,
  filters,
      ...
      <Box sx={styles.stickyBar}>
          ...
          <Box sx={{ display: "flex", flexFlow: "row wrap", flexGrow: 1 }}>
            ...
            {filters.banos && (
              <Chip
                ...
                sx={styles.chip}
                label={`${filters.banos} ${
                  Number(filters.banos) !== 1 ? "Baños" : "Baño"
                }`}
              />
            )}
            {filters.habitaciones && (
              <Chip
                size="small"
                color="primary"
                onDelete={() => onDelete("habitaciones")}
                variant="outlined"
                sx={styles.chip}
            ...
            {filters.estacionamientos && (
              <Chip
                size="small"
                color="primary"
                onDelete={() => onDelete("estacionamientos")}
                variant="outlined"
                sx={styles.chip}
                label={`${filters.estacionamientos} Est.`}
              />
            )}
            {filters.query && (
              <Chip
            ...
            {Number(filters.from) > 0 && (
              <Chip
                size="small"
                color="primary"
                onDelete={() => onDelete("from")}
                variant="outlined"
                sx={styles.chip}
                label={`Min. ${filters.from}`}
              />
            )}
            {Number(filters.to) > 0 && (
              <Chip
                size="small"
                color="primary"
                onDelete={() => onDelete("to")}
                variant="outlined"
                sx={styles.chip}
                label={`Max. ${filters.to}`}
              />
            )}
            {filters.negocio && (
              <Chip
                size="small"
                color="primary"
                onDelete={() => onDelete("negocio")}
                variant="outlined"
                sx={styles.chip}
                label={`${filters.negocio}`}
              />
            )}
            {filters.localidad && (
              <Chip
                size="small"
                color="primary"
                onDelete={() => onDelete("localidad")}
                variant="outlined"
                sx={styles.chip}
                label={`${filters.localidad}`}
              />
            )}
            {filters.tipo && (
              <Chip
        ...
        <Box
          sx={{
            ...
            flexGrow: 1,
            transform: search ? "scale(1)" : "scale(0)",
            mt: 1,
            mb: 1,
          }}
        >
          <form onSubmit={onSubmit}>
            <TextField
              ...
              onChange={(e: ChangeEvent<HTMLInputElement>) =>
                setNewQuery(String(e.target.value))
              }
              value={newQuery}
              sx={{
                "& input": {
