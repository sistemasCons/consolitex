...
import { SeccionSuperior, EndMessage } from "./";
...
export const InmuebleList: FC<Props> = ({
  inmueblesSSR,
  localidadSSR = "",
  querySSR = "",
  negocioSSR = "",
  ...
  const InmuebleCard = dynamic(() =>
    import("./").then((mod) => mod.InmuebleCard)
  );
  const InmuebleCardGrid = dynamic(() =>
    import("./").then((mod) => mod.InmuebleCardGrid)
  );
  ...
  const [lastItemKey, setLastItemKey] = useState<any>(
    inmueblesSSR ? inmueblesSSR[inmueblesSSR.length - 1].data.key : 0
  );

  // Si está en true, al hacer scroll infinito se buscan más inmuebles
  const [hasMore, setHasMore] = useState<any>(
    inmueblesSSR && inmueblesSSR.length < 20 ? false : true
  );
  ...
  const fetchData = async () => {
    ...
    if (
      filters.from &&
      Number(filters.from.toString().replace(/[^0-9]/g, "")) !== 0
    ) {
      params.push([
        "from",
        String(filters.from.toString().replace(/[^0-9]/g, "")),
      ]);
    }
    if (
      filters.to &&
      Number(filters.to.toString().replace(/[^0-9]/g, "")) !== 0
    ) {
    ...
    const url =
      params.length > 0
        ? new URL(
            `/api/infiniteScroll?lastItem=${lastItemKey}`,
            window.location.origin
          )
        : new URL(`/api/filter`, window.location.origin);
  ...
  return (
    <Box sx={styles.mainContainer}>
      ...
      <InfinityScroll
        dataLength={
          inmueblesState && inmueblesState.length > 0
            ? inmueblesState.length
            : 0
        }
        hasMore={hasMore}
        loader={<LinearProgress />}
        endMessage={<EndMessage fetchNewData={fetchNewData} />}
        next={fetchData}
      >
        {inmueblesState?.length > 0 ? (
          squared ? (
            inmueblesState.map((inmueble: Inmueble) => (
              <Suspense
                fallback={<PlaceholderNoGrid />}
                key={inmueble.data.key}
              >
